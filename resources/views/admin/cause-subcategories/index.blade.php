...
@section('title', 'تصنيف الأسباب - الفرعي')
@section('page-title', 'تصنيف الأسباب - الفرعي (CAUSE_SUBCATEGORY — L3)')
@section('breadcrumbs')
  <li class="breadcrumb-item">البيانات المرجعية</li>
  <li class="breadcrumb-item active">تصنيف الأسباب - الفرعي</li>
@endsection

@section('content')
<div class="card">
  <div class="card-body">
    <h5 class="card-title">إضافة تصنيف فرعي جديد للسبب</h5>
    <form method="POST" action="{{ route('admin.cause-subcategories.store') }}" class="row g-2 mb-4">
      @csrf
      <div class="col-md-3">
        <select name="cause_category_id" class="form-select" required>
          <option value="">-- فئة السبب --</option>
    ...
    <div class="table-responsive">
      <table class="table table-striped datatable">
        <thead><tr><th>فئة السبب</th><th>الكود</th><th>التصنيف الفرعي</th><th>الحالة</th><th></th></tr></thead>
        <tbody>
          @foreach($subcategories as $subcategory)
            <tr>
              <td>{{ $subcategory->causeCategory?->category_name }}</td>
              ...
              <td>
                <form method="POST" action="{{ route('admin.cause-subcategories.toggle', $subcategory) }}">
                  @csrf
